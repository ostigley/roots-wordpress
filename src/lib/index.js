// Generated by CoffeeScript 1.10.0
(function() {
  (function() {
    var API, RootsUtil, W, _, add_posts_to_locals, add_urls_to_posts, create_tag_tree, path, render_single_views, render_tag_views, request, results_obj;
    API = require('./api');
    W = require('when');
    RootsUtil = require('roots-util');
    _ = require('lodash');
    path = require('path');
    request = require('request');
    results_obj = {};
    module.exports = function(opts) {
      var RootsWordpress;
      RootsWordpress = void 0;
      if (opts === null) {
        opts = {};
      }
      if (!opts.site) {
        throw new Error('You must supply a site url or id');
      }
      if (!opts.post_types) {
        opts.post_types = {
          post: {}
        };
      }
      return RootsWordpress = (function() {
        var RootsWordpress;
        RootsWordpress = function(roots) {
          this.roots = roots;
          this.util = new RootsUtil(this.roots);
          opts.site = opts.site.replace(/http:\/\//, '');
        };
        RootsWordpress.prototype.setup = function() {
          var _base, all, config, type;
          all = void 0;
          config = void 0;
          type = void 0;
          _base = void 0;
          if ((_base = this.roots.config).locals === null) {
            _base.locals = {};
          }
          this.roots.config.locals.wordpress = {};
          all = (function() {
            var type;
            var _ref, _results, posts, render_tags, tag_tree;
            _ref = void 0;
            _results = void 0;
            _ref = opts.post_types;
            _results = [];
            posts = void 0;
            tag_tree = void 0;
            render_tags = render_tag_views.bind(this);
            for (type in _ref) {
              config = _ref[type];
              _results.push(request(opts.site, type, config).then((function(res) {
                posts = res.entity.posts;
                tag_tree = create_tag_tree(posts);
                W(tag_tree).then(render_single_views.bind(this, config, type, res)).then(add_urls_to_posts).then(add_posts_to_locals.bind(this, type)).then(render_tags.bind(this, config.tag_tree, tag_tree)).then(add_posts_to_locals.bind(this, 'tag_tree', tag_tree));
              }).bind(this)));
            }
            return _results;
          }).call(this);
          return W.all(all);
        };
        return RootsWordpress;
      })();
    };
    request = function(site, type, config) {
      var params;
      params = void 0;
      params = _.merge(config, {
        type: type
      });
      return API({
        path: '' + site + '/posts',
        params: params
      });
    };
    render_single_views = function(config, type, res) {
      var posts;
      posts = void 0;
      posts = res.entity.posts;
      if (!config.template) {
        return {
          urls: [],
          posts: posts
        };
      }
      return W.map(posts, (function(_this) {
        return function(p) {
          var compiler, locals, output, tpl;
          compiler = void 0;
          locals = void 0;
          output = void 0;
          tpl = void 0;
          tpl = path.join(_this.roots.root, config.template);
          locals = _.merge(_this.roots.config.locals, {
            post: p
          });
          output = '' + type + '/' + p.slug + '.html';
          compiler = _.find(_this.roots.config.compilers, function(c) {
            return _.contains(c.extensions, path.extname(tpl).substring(1));
          });
          return compiler.renderFile(tpl, _.cloneDeep(locals)).then(function(res) {
            return _this.util.write(output, res.result);
          })['yield'](output);
        };
      })(this)).then(function(urls) {
        results_obj = {
          urls: urls,
          posts: posts
        };
        return results_obj;
      });
    };
    add_urls_to_posts = function(obj) {
      return obj.posts.map(function(post, i) {
        post._url = obj.urls[i];
        return post;
      });
    };
    add_posts_to_locals = function(type, posts) {
      return this.roots.config.locals.wordpress[type] = posts;
    };
    render_tag_views = function(config, tag_tree, obj) {
      var tags;
      tags = Object.keys(tag_tree);
      return W.map(tags, (function(_this) {
        return function(p) {
          var compiler, locals, output, tpl;
          compiler = void 0;
          locals = void 0;
          output = void 0;
          tpl = void 0;
          tpl = path.join(_this.roots.root, config.template);
          locals = _this.roots.config.locals;
          locals.tag = tag_tree[p];
          output = 'tags/' + p + '.html';
          compiler = _.find(_this.roots.config.compilers, function(c) {
            return _.contains(c.extensions, path.extname(tpl).substring(1));
          });
          return compiler.renderFile(tpl, _.cloneDeep(locals)).then(function(res) {
            return _this.util.write(output, res.result);
          })['yield'](output);
        };
      })(this)).then(function(urls) {
        urls.map(function(url) {
          return results_obj.urls.push(url);
        });
        return results_obj;
      });
    };
    create_tag_tree = function(posts) {
      var tag_tree;
      tag_tree = {};
      posts.map(function(post) {
        Object.keys(post.tags).map(function(tag) {
          var slug;
          slug = post.tags[tag].slug;
          if (!tag_tree[slug]) {
            tag_tree[slug] = {
              posts: [post],
              slug: slug
            };
          } else {
            tag_tree[slug].posts.push(post);
          }
        });
      });
      return tag_tree;
    };
  }).call(this);

}).call(this);
